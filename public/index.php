...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button aria-label="Submit search" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center">
      <p class="text-slate-400 text-lg">No movies found matching your criteria.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="mt-6 inline-block rounded-xl border border-slate-700 px-6 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition">Clear Search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
