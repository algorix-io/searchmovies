...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center py-12 rounded-2xl border border-slate-800 bg-slate-900/50">
      ...
      <p class="text-slate-400 mb-6"><?= $q !== '' ? 'We couldn\'t find any movies matching "' . h($q) . '".' : 'There are currently no movies available.' ?></p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-flex items-center justify-center rounded-xl bg-slate-800 border border-slate-700 px-6 py-2 hover:bg-slate-700 hover:border-slate-600 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
