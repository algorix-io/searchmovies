...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-10 text-center py-10 rounded-2xl border border-slate-800 border-dashed">
      <div class="text-4xl mb-4">📭</div>
      <h2 class="text-xl font-semibold mb-2">No movies found</h2>
      <?php if($q !== ''): ?>
        <p class="text-slate-400 mb-4">We couldn't find anything matching "<?=h($q)?>".</p>
        <a href="/" class="inline-block rounded-xl bg-slate-800 px-5 py-2 hover:bg-slate-700 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Clear search</a>
      <?php else: ?>
        <p class="text-slate-400">There are no movies available at the moment.</p>
      <?php endif; ?>
    </div>
  <?php else: ?>
