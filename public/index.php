...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <a href="/" class="text-xl font-extrabold brand-grad outline-none focus-visible:ring focus-visible:ring-sky-600 rounded">🎬 <?=h(APP_NAME)?></a>
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if(empty($movies)): ?>
      <div class="col-span-full py-16 px-4 text-center bg-slate-900/40 rounded-3xl border border-slate-800">
        <div class="text-5xl mb-4 opacity-80">🔍</div>
        <h2 class="text-2xl font-semibold mb-2">No movies found</h2>
        <p class="text-slate-400 mb-6 max-w-md mx-auto">We couldn't find any movies matching "<?=h($q)?>". Try checking for typos or using different keywords.</p>
        <a href="/" class="inline-block rounded-xl bg-slate-800 px-6 py-2.5 font-medium hover:bg-slate-700 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Clear Search</a>
      </div>
    <?php else: ?>
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">
