...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <a href="/" class="text-xl font-extrabold brand-grad outline-none focus:ring focus:ring-sky-600 rounded px-1 hover:opacity-80 transition">🎬 <?=h(APP_NAME)?></a>
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" aria-label="Search movies" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600 transition">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 transition outline-none focus:ring focus:ring-sky-600">
...
<footer class="border-t border-slate-800">
  <div class="max-w-6xl mx-auto px-4 py-6 text-slate-400 text-sm flex flex-wrap items-center gap-3">
    ...
    <span class="footer-tag">Powered by <a href="https://knowhub.uz" target="_blank" rel="noopener" class="underline hover:text-white outline-none focus:ring focus:ring-sky-600 rounded px-1 transition">KnowHub.uz</a></span>
  </div>
</footer>
</body></html>
