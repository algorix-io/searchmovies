...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <a href="/" class="text-xl font-extrabold brand-grad outline-none focus:ring focus:ring-sky-600 rounded">🎬 <?=h(APP_NAME)?></a>
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center text-slate-400">
      ...
      <a href="/" class="mt-6 inline-block rounded-xl bg-slate-800 px-6 py-2 text-sm font-medium hover:bg-slate-700 outline-none focus:ring focus:ring-sky-600">Clear Search</a>
    </div>
  <?php else: ?>
