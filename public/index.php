...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" aria-label="Search movies" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600">
      <button type="submit" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center py-12 border border-dashed border-slate-800 rounded-2xl bg-slate-900/50">
      <div class="text-4xl mb-3" aria-hidden="true">🕵️‍♂️</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <?php if($q): ?>
        <p class="text-slate-400 mt-2 max-w-md mx-auto">We couldn't find anything matching "<?=h($q)?>". Try adjusting your search terms.</p>
        <div class="mt-6">
          <a href="/" class="inline-flex items-center gap-2 rounded-xl bg-sky-600 text-white px-5 py-2.5 hover:bg-sky-500 transition font-medium focus:ring focus:ring-sky-600 outline-none">
            Clear Search
          </a>
        </div>
      <?php else: ?>
        <p class="text-slate-400 mt-2">There are no movies in the database yet. Check back later!</p>
      <?php endif; ?>
    </div>
  <?php else: ?>
