...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form role="search" class="flex gap-2 w-full max-w-lg" method="get">
...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 outline-none focus:ring focus:ring-sky-600 focus:border-sky-600 transition">
...
<footer class="border-t border-slate-800">
  <div class="max-w-6xl mx-auto px-4 py-6 text-slate-400 text-sm flex flex-wrap items-center gap-3">
    ...
    <span class="footer-tag">Powered by <a href="https://knowhub.uz" target="_blank" rel="noopener" class="underline outline-none focus:ring focus:ring-sky-600 rounded transition hover:text-white">KnowHub.uz</a></span>
  </div>
</footer>
</body></html>
