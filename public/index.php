...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
  <div class="mt-12 text-center py-12 px-4 rounded-2xl border border-slate-800 bg-slate-900/50">
    ...
    <a href="/" class="inline-block rounded-xl bg-sky-600 text-white px-6 py-2.5 font-medium hover:bg-sky-700 outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 transition">Clear search</a>
  </div>
  <?php else: ?>
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 outline-none focus-visible:ring-2 focus-visible:ring-sky-600 transition group">
