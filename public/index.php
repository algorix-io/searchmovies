...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if (empty($movies) && $q !== ''): ?>
      <div class="col-span-full py-12 text-center bg-slate-900/50 rounded-2xl border border-slate-800">
        <div class="text-4xl mb-4">🔍</div>
        <h2 class="text-xl font-semibold mb-2">No movies found</h2>
        <p class="text-slate-400 mb-6">We couldn't find anything matching "<?=h($q)?>". Try different keywords or browse all movies.</p>
        <a href="/" class="inline-block bg-sky-600 hover:bg-sky-500 text-white font-medium px-6 py-2 rounded-xl transition outline-none focus-visible:ring focus-visible:ring-sky-400">Clear Search</a>
      </div>
    <?php elseif (empty($movies)): ?>
      <div class="col-span-full py-12 text-center bg-slate-900/50 rounded-2xl border border-slate-800">
        <div class="text-4xl mb-4">🎬</div>
        <h2 class="text-xl font-semibold mb-2">No movies available</h2>
