...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition-colors">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center flex flex-col items-center">
      ...
      <p class="text-slate-400 max-w-md">We couldn't find any movies matching your criteria. Try adjusting your search or browse our catalog.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="mt-6 inline-flex items-center gap-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white px-5 py-2.5 font-medium transition-colors outline-none focus:ring focus:ring-sky-400">
          Clear search
        </a>
      <?php endif; ?>
    </div>
