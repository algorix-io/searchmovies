...
<main class="max-w-6xl mx-auto px-4 py-6">
  <div class="flex items-end justify-between">
    <h1 class="text-2xl font-bold"><?= $q !== '' ? 'Search Results for "'.h($q).'"' : 'Fresh Movies' ?></h1>
    <div class="text-slate-400 text-sm"><?=count($movies)?> results</div>
  </div>
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center py-12 rounded-2xl border border-slate-800 border-dashed">
      <div class="text-4xl mb-4">🍿</div>
      <h2 class="text-xl font-semibold mb-2">No movies found</h2>
      <p class="text-slate-400 mb-6">We couldn't find anything matching your search.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-block rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition outline-none focus:ring focus:ring-sky-600">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
