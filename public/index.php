...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if(empty($movies)): ?>
      <div class="col-span-full py-12 text-center text-slate-400 border border-dashed border-slate-800 rounded-2xl">
        <div class="text-4xl mb-4" aria-hidden="true">🔍</div>
        <h2 class="text-xl font-semibold text-slate-200 mb-2">No movies found</h2>
        <?php if($q !== ''): ?>
          <p class="mb-4">We couldn't find any movies matching "<?=h($q)?>".</p>
          <a href="/" class="inline-flex items-center justify-center rounded-xl bg-slate-800 px-4 py-2 text-sm font-medium hover:bg-slate-700 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Clear search</a>
        <?php else: ?>
