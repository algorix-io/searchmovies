...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 outline-none focus:ring focus:ring-sky-600 hover:bg-slate-800 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if(empty($movies)): ?>
      <div class="col-span-full py-12 text-center border border-slate-800 border-dashed rounded-2xl bg-slate-900/50">
        <div class="text-4xl mb-3"><?= $q !== '' ? '🔍' : '🎬' ?></div>
        <h2 class="text-xl font-semibold mb-2"><?= $q !== '' ? 'No movies found' : 'No movies available' ?></h2>
        <?php if($q !== ''): ?>
          <p class="text-slate-400 mb-5">We couldn't find any movies matching "<?=h($q)?>".</p>
          <a href="/" class="inline-flex items-center justify-center rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 px-4 py-2 outline-none focus:ring focus:ring-sky-600 transition">Clear search</a>
        <?php else: ?>
