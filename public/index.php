...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 active:bg-slate-700 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center text-slate-400">
      ...
      <?php if($q !== ''): ?>
        <a href="/" class="mt-4 inline-block rounded-xl bg-slate-800 px-4 py-2 hover:bg-slate-700 transition outline-none focus-visible:ring focus-visible:ring-sky-600 text-slate-200">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
