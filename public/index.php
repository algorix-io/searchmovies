...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      <input type="search" name="q" aria-label="Search movies" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus-visible:ring focus-visible:ring-sky-600">
      <button type="submit" aria-label="Submit search" class="rounded-xl bg-slate-800 border border-slate-700 px-4 py-2 hover:bg-slate-700 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 flex flex-col items-center justify-center text-center p-8 border border-slate-800 rounded-2xl border-dashed">
      <div class="text-6xl mb-4">🎬</div>
      <h2 class="text-xl font-bold mb-2">No movies found</h2>
      <p class="text-slate-400 mb-6">We couldn't find any movies matching "<?=h($q)?>". Try a different search term.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="px-6 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 font-medium transition outline-none focus-visible:ring focus-visible:ring-sky-600">
          Clear Search
        </a>
      <?php endif; ?>
    </div>
