...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      <input aria-label="Search movies" type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600 focus-visible:ring focus-visible:ring-sky-600">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus-visible:ring focus-visible:ring-sky-600 transition-colors">Search</button>
    </form>
  </div>
</header>
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center">
      ...
      <p class="text-slate-400 mb-6">
        <?php if($q !== ''): ?>
          We couldn't find any movies matching "<?=h($q)?>". Try a different search term.
        <?php else: ?>
          There are no movies available right now. Please check back later.
        <?php endif; ?>
      </p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2 hover:border-sky-600 outline-none focus-visible:ring focus-visible:ring-sky-600 transition-colors">
          Clear search
        </a>
      <?php endif; ?>
    </div>
