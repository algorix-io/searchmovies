...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-8 text-center py-12 rounded-2xl border border-slate-800 bg-slate-900/50">
      <div class="text-5xl mb-4">🔍</div>
      <h2 class="text-xl font-semibold mb-2">No movies found</h2>
      <p class="text-slate-400 mb-6">
        <?= $q !== '' ? 'We couldn\'t find any movies matching "'.h($q).'".' : 'Check back later for new movies.' ?>
      </p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-block rounded-xl bg-slate-800 px-5 py-2 hover:bg-slate-700 outline-none focus-visible:ring focus-visible:ring-sky-600 transition-colors">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
