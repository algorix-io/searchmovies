...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." aria-label="Search movies" class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600 focus-visible:ring-2 focus-visible:ring-sky-500">
...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="py-20 text-center">
      <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-800 mb-4 text-4xl" role="img" aria-label="Sad face">😕</div>
      <h3 class="text-xl font-bold mb-2">No movies found</h3>
      <p class="text-slate-400 mb-6 max-w-md mx-auto">
        We couldn't find any movies matching "<?=h($q)?>".<br>Try adjusting your search or browse all movies.
      </p>
      <a href="/" class="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-6 py-2.5 font-semibold text-white hover:bg-sky-500 transition">
        View all movies
      </a>
    </div>
  <?php else: ?>
