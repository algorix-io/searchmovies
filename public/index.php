...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center py-12 rounded-2xl border border-dashed border-slate-700 bg-slate-900/50">
      <div class="text-4xl mb-4 opacity-50">🍿</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <p class="text-slate-400 mt-2">We couldn't find any movies matching "<?=h($q)?>".</p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-block mt-4 text-sky-400 hover:text-sky-300 underline outline-none focus:ring focus:ring-sky-600 rounded-lg px-2 py-1 transition">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
