...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button aria-label="Search movies" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition outline-none focus:ring focus:ring-sky-600 hover:cursor-pointer">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-16 text-center">
      <div class="text-slate-400 text-lg">No movies found <?php if($q!==''): ?>for "<span class="text-slate-100"><?=h($q)?></span>"<?php endif; ?></div>
      <a href="/" class="inline-block mt-4 px-4 py-2 rounded-xl border border-slate-700 hover:bg-slate-800 transition outline-none focus:ring focus:ring-sky-600 hover:cursor-pointer">Clear Search</a>
    </div>
  <?php else: ?>
