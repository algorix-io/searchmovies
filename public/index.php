...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 focus:outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 text-center">
      ...
      <a href="/" class="inline-block rounded-xl border border-slate-700 px-6 py-2 hover:bg-slate-800 focus:outline-none focus:ring focus:ring-sky-600 transition">Clear Search</a>
    </div>
  <?php else: ?>
    <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      <?php foreach($movies as $m): ?>
        <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-600 focus:ring-offset-2 focus:ring-offset-slate-950 transition">
