  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if (empty($movies)): ?>
      <div class="col-span-full py-12 text-center text-slate-400">
        ...
        <p>We couldn't find any movies<?php if ($q !== '') echo ' matching "'.h($q).'"'; ?>.</p>
        <?php if ($q !== ''): ?>
          <a href="/" class="inline-block mt-4 rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition text-slate-200">Clear search</a>
        <?php endif; ?>
      </div>
    <?php else: ?>
