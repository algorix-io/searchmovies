...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 focus-visible:ring focus-visible:ring-sky-600 outline-none transition">Search</button>
    </form>
  </div>
</header>
    ...
    <?php if (empty($movies)): ?>
      <div class="col-span-full py-12 text-center">
        ...
        <?php if ($q !== ''): ?>
          ...
          <a href="/" class="mt-4 inline-block text-sky-400 hover:text-sky-300 underline outline-none focus-visible:ring focus-visible:ring-sky-600 rounded">Clear search</a>
        <?php else: ?>
          <p class="text-slate-400">There are no movies available right now.</p>
        <?php endif; ?>
      </div>
    <?php else: ?>
