...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if (empty($movies)): ?>
    <div class="mt-12 text-center py-12 rounded-2xl border border-slate-800 bg-slate-900/50">
      ...
      <?php if ($q !== ''): ?>
        <a href="/" class="inline-block rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 hover:shadow-lg hover:-translate-y-1 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">
