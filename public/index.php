...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(count($movies) === 0): ?>
    <div class="mt-12 text-center py-12 rounded-2xl border border-slate-800 bg-slate-900/50">
      <div class="text-4xl mb-4">🍿</div>
      <h2 class="text-xl font-semibold mb-2">No movies found</h2>
      <p class="text-slate-400 mb-6">
        <?php if($q !== ''): ?>
          We couldn't find any movies matching "<?=h($q)?>".
        <?php else: ?>
          There are currently no movies available.
        <?php endif; ?>
      </p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-flex items-center gap-2 rounded-xl bg-slate-800 border border-slate-700 px-4 py-2 hover:bg-slate-700 transition outline-none focus-visible:ring focus-visible:ring-sky-600">
          Clear search
        </a>
      <?php endif; ?>
    </div>
