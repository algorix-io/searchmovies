...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 hover:bg-slate-700 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
  <div class="mt-12 py-12 text-center border border-slate-800 border-dashed rounded-2xl bg-slate-900/50">
    <div class="text-4xl mb-4">🔍</div>
    <h2 class="text-xl font-semibold mb-2">No movies found</h2>
    <p class="text-slate-400">We couldn't find any movies matching "<?=h($q)?>".</p>
    <?php if($q !== ''): ?>
    <a href="/" class="inline-block mt-4 text-sky-500 hover:text-sky-400 underline outline-none focus-visible:ring focus-visible:ring-sky-600 rounded transition">Clear search</a>
    <?php endif; ?>
  </div>
  <?php else: ?>
