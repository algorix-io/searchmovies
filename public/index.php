...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." aria-label="Search movies" class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition-colors outline-none focus-visible:ring focus-visible:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if (empty($movies)): ?>
      <div class="col-span-full py-12 text-center border border-slate-800 rounded-2xl bg-slate-900/50">
        <div class="text-4xl mb-4">🔍</div>
        <h2 class="text-xl font-semibold mb-2">No movies found</h2>
        <p class="text-slate-400 mb-6">We couldn't find any movies matching "<?=h($q)?>".</p>
        <a href="/" class="inline-flex rounded-xl bg-sky-600 text-white px-6 py-2.5 font-medium hover:bg-sky-500 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950">Clear search</a>
      </div>
    <?php else: ?>
