...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-16 mb-16 text-center flex flex-col items-center">
      <div class="text-6xl mb-4 opacity-80" aria-hidden="true">🍿</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <?php if($q !== ''): ?>
        <p class="text-slate-400 mt-2 max-w-md">We couldn't find any movies matching "<span class="text-slate-200"><?=h($q)?></span>". Try a different search term.</p>
        <a href="/" class="mt-6 rounded-xl border border-slate-700 px-6 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition-colors">Clear search</a>
      <?php else: ?>
        <p class="text-slate-400 mt-2">There are currently no movies available.</p>
      <?php endif; ?>
    </div>
  <?php else: ?>
