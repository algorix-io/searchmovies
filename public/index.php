...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 focus-visible:ring-2 focus-visible:ring-sky-600 outline-none">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies) && $q !== ''): ?>
    <div class="mt-8 text-center bg-slate-900/50 rounded-2xl border border-slate-800 p-8">
      ...
      <a href="/" class="inline-flex items-center gap-2 rounded-xl bg-sky-600 text-white px-5 py-2.5 font-medium hover:bg-sky-500 focus-visible:ring-2 focus-visible:ring-sky-400 outline-none transition">
        Clear search
      </a>
    </div>
  <?php else: ?>
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 focus-visible:ring-2 focus-visible:ring-sky-600 outline-none transition">
