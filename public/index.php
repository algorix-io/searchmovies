...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      <input type="search" name="q" aria-label="Search movies" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600 focus:border-sky-600 transition-all">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 hover:border-slate-500 outline-none focus:ring focus:ring-sky-600 focus:border-sky-600 transition-all">Search</button>
    </form>
  </div>
</header>
