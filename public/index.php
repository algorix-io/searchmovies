  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if (empty($movies)): ?>
      <div class="col-span-full py-16 text-center border border-dashed border-slate-800 rounded-2xl">
        <p class="text-xl font-semibold text-slate-300">No movies found</p>
        <p class="mt-2 text-slate-400">
          <?php if ($q): ?>
            We couldn't find anything matching "<?=h($q)?>". <br>
            <a href="/" class="mt-3 inline-block text-sky-500 hover:text-sky-400 hover:underline outline-none focus:ring focus:ring-sky-600 rounded transition">Clear search</a>
          <?php else: ?>
            Check back later for fresh movies!
          <?php endif; ?>
        </p>
      </div>
    <?php else: ?>
