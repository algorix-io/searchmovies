...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." aria-label="Search movies" class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus-visible:ring focus-visible:ring-sky-600">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if (empty($movies)): ?>
    <div class="mt-12 text-center text-slate-400">
      ...
      <?php if($q !== ''): ?>
        <p class="mt-2">We couldn't find any movies matching "<?=h($q)?>".</p>
        <a href="/" class="inline-block mt-4 px-4 py-2 rounded-xl border border-slate-700 hover:bg-slate-800 transition outline-none focus-visible:ring focus-visible:ring-sky-600 text-slate-200">Clear search</a>
      <?php else: ?>
        <p class="mt-2">Check back later for new additions.</p>
      <?php endif; ?>
    </div>
  <?php else: ?>
