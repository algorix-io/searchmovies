...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <label for="search-movies" class="sr-only">Search movies</label>
      <input id="search-movies" type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600">
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 hover:border-sky-600 outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 py-12 text-center rounded-2xl border border-slate-800 bg-slate-900/50">
      <div class="text-4xl mb-4">🔍</div>
      <h2 class="text-xl font-semibold mb-2">No movies found</h2>
      <?php if($q !== ''): ?>
        <p class="text-slate-400 mb-6">We couldn't find anything matching "<?=h($q)?>".</p>
        <a href="/" class="inline-block rounded-xl border border-slate-700 px-6 py-2 hover:bg-slate-800 hover:border-sky-600 outline-none focus:ring focus:ring-sky-600 transition">Clear Search</a>
      <?php else: ?>
