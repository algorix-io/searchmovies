...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" aria-label="Search movies" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus-visible:ring focus-visible:ring-sky-600">
      <button aria-label="Submit search" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus-visible:ring focus-visible:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if (empty($movies)): ?>
      <div class="col-span-full py-12 text-center text-slate-400">
        ...
        <a href="/" class="mt-6 inline-block rounded-xl bg-sky-600 text-white px-6 py-2 hover:bg-sky-500 outline-none focus-visible:ring focus-visible:ring-sky-400 transition">Clear Search</a>
      </div>
    <?php else: ?>
