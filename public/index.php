...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600 transition">
      <button class="rounded-xl bg-slate-900 border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="py-16 text-center border border-slate-800 border-dashed rounded-2xl mt-8">
      <div class="text-5xl mb-4" aria-hidden="true">📭</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <p class="mt-2 text-slate-400">We couldn't find any movies matching your search.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-block mt-6 px-5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 hover:bg-slate-800 hover:border-slate-600 outline-none focus:ring focus:ring-sky-600 transition font-medium text-slate-200">Clear search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
    <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      <?php foreach($movies as $m): ?>
        <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 outline-none focus:ring focus:ring-sky-600 transition group">
          <img src="<?=h($m['poster_url'] ?: 'https://placehold.co/600x900/png')?>" alt="<?=h($m['title'])?> poster" class="w-full h-80 object-cover group-hover:opacity-90 transition">
          <div class="p-3">
