...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get" role="search">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 outline-none focus-visible:ring-2 focus-visible:ring-sky-600 transition-colors">Search</button>
    </form>
  </div>
</header>
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php if(empty($movies)): ?>
      <div class="col-span-full py-12 text-center text-slate-400">
        ...
        <h2 class="text-xl font-semibold text-slate-300">No movies found</h2>
        <p class="mt-2">We couldn't find any movies matching your search.</p>
        <?php if($q !== ''): ?>
          <a href="/" class="inline-block mt-4 px-4 py-2 rounded-xl border border-slate-700 hover:border-sky-600 hover:bg-slate-800 transition outline-none focus-visible:ring-2 focus-visible:ring-sky-600">Clear search</a>
        <?php endif; ?>
      </div>
    <?php else: ?>
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 outline-none focus-visible:ring-2 focus-visible:ring-sky-600 transition">
