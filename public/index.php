...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if (empty($movies)): ?>
    <div class="mt-8 py-16 text-center rounded-2xl border border-slate-800 border-dashed bg-slate-900/30">
      <div class="text-4xl mb-4">🔍</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <p class="mt-2 text-slate-400"><?= $q !== '' ? 'We couldn\'t find any movies matching "'.h($q).'".' : 'No movies are currently available.' ?></p>
      <?php if ($q !== ''): ?>
        <a href="/" class="mt-6 inline-block rounded-xl border border-slate-700 bg-slate-800 px-6 py-2 font-medium hover:bg-slate-700 outline-none focus:ring focus:ring-sky-600 transition">Clear Search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
