...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      <input aria-label="Search movies" type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring focus:ring-sky-600">
      <button aria-label="Submit search" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 transition outline-none focus-visible:ring focus-visible:ring-sky-600">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-12 flex flex-col items-center justify-center text-center">
      ...
      <h2 class="text-2xl font-bold mb-2"><?= $q !== '' ? 'No movies found' : 'No movies available' ?></h2>
      <p class="text-slate-400 mb-6"><?= $q !== '' ? 'Try adjusting your search terms.' : 'Check back later for new additions.' ?></p>
      <?php if($q !== ''): ?>
        <a href="/" class="inline-flex items-center justify-center rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-medium px-6 py-2.5 transition outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 focus-visible:ring-sky-500">
          Clear Search
        </a>
      <?php endif; ?>
    </div>
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 transition outline-none focus-visible:ring focus-visible:ring-sky-600">
