...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form class="flex gap-2 w-full max-w-lg" method="get">
      ...
      <button class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 hover:border-sky-600 focus:outline-none focus:ring focus:ring-sky-600 transition">Search</button>
    </form>
  </div>
</header>
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    <?php foreach($movies as $m): ?>
      <a href="/movie/<?=h($m['slug'])?>" class="block rounded-2xl overflow-hidden border border-slate-800 hover:border-sky-600 focus:outline-none focus:ring focus:ring-sky-600 transition">
  ...
  <?php else: ?>
  <div class="mt-12 text-center py-12 px-4 rounded-2xl border border-slate-800/50 bg-slate-900/30">
    <div class="text-4xl mb-4">🎬</div>
    <h2 class="text-xl font-semibold mb-2">No movies found</h2>
    <p class="text-slate-400 max-w-md mx-auto mb-6">We couldn't find any movies matching your criteria. Try adjusting your search or browse our latest additions.</p>
    <?php if($q): ?>
      <a href="/" class="inline-block rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-medium px-6 py-2 focus:outline-none focus:ring focus:ring-sky-600 focus:ring-offset-2 focus:ring-offset-slate-950 transition">Clear Search</a>
    <?php endif; ?>
  </div>
  <?php endif; ?>
</main>
