...
<header class="border-b border-slate-800 sticky top-0 bg-slate-950/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <form role="search" class="flex gap-2 w-full max-w-lg" method="get" action="/">
      <label for="search-q" class="sr-only">Search movies</label>
      <input id="search-q" type="search" name="q" value="<?=h($q)?>" placeholder="Search movies..." class="flex-1 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 outline-none focus:ring-2 focus:ring-sky-600 transition-colors">
      <button type="submit" class="rounded-xl border border-slate-700 px-4 py-2 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-sky-600 transition-colors font-medium">Search</button>
    </form>
  </div>
</header>
