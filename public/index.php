...
<main class="max-w-6xl mx-auto px-4 py-6">
  ...
  <?php if(empty($movies)): ?>
    <div class="mt-8 rounded-2xl border border-dashed border-slate-700 bg-slate-900/50 p-12 text-center text-slate-400">
      <div class="text-4xl mb-3">🔍</div>
      <h2 class="text-xl font-semibold text-slate-200">No movies found</h2>
      <p class="mt-2">Try adjusting your search query.</p>
      <?php if($q !== ''): ?>
        <a href="/" class="mt-4 inline-block rounded-xl bg-sky-600 px-6 py-2 font-medium text-white hover:bg-sky-500 outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-slate-950 transition">Clear Search</a>
      <?php endif; ?>
    </div>
  <?php else: ?>
